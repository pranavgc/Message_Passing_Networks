...
columns=["index","name", "x", "y", "z","l"]
s=pd.read_csv('/home/pranav/ANN_Model/subgraph_generation/GNN_final_data/cellIndex.txt',sep=" ",usecols=columns)

#cell= np.array([s['x'],s['y'],s['z'],s['l'],s['index']])
cell= np.array([s['x'],s['y'],s['z'],s['l']])
cell=np.transpose(cell)


class CustomDataset(Dataset):
    def __init__(self, labels, nodes,mark, edges, edge_a, transform=None, target_transform=None,add_self_loops: bool = True,prominent:bool=True,add_xy:bool=False,add_dEdz:bool=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.add_dEdz:
            dE=[]
            for i in range(len(self.nodes[idx][:,0])):
                dE.append(data_n[i,0]/data_n[i,1])
                
            dE=torch.transpose(torch.tensor([dE], dtype=torch.float32),0,1)
           
            x1=torch.cat((x1,dE),dim=1)
        "Maximal or not"
        if self.prominent:
...
'''class GCN_v2(MessagePassing):
    ...
    def message(self, x_j, x_i,edge_weight,norm):
        ...
        #in_f=torch.cat((x_i,x_j),dim=1)
        in_f1=x_i
        in_f2=x_i-x_j
        out1=self.mlp1(in_f1)
        out2=self.mlp2(in_f2)
        out_2=torch.exp(self.jack)*edge_weight.view(-1,1)*out2
        in_f=torch.cat((out1,out2),dim=1,out=None)
        #in_f=torch.reshape(in_f,(1,len(in_f)))
        #in_f=x_j
        #print("\nshape input in_f",in_f.shape)
        out_f=self.mlp3(in_f)
...
#without dropout.dropout only at end of convolution.    
class GCN_v3(MessagePassing):
...
class MP_GNN(torch.nn.Module):
    def __init__(self,input_channels, hidden_channels,num_classes,num_conv_layers,dropout, pool):
        ...
        self.convs = nn.ModuleList([GCN_v3(hidden_channels,hidden_channels)])
        self.convs.extend([GCN_v3(hidden_channels,hidden_channels) for i in range(num_conv_layers)])
